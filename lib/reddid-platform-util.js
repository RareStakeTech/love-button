...
(function () {
  ...
  function tryLookup(platform, username, onFound) {
    // Fetch the configured API base once before doing identity lookups
   ...
   * @returns {string}
   */
  function tipUrl(identity, base) {
    return `${base || API_BASE}/${identity.handle}`;
  }
